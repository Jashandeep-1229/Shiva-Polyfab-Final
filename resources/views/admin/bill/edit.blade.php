...
@section('breadcrumb-items')
    ...
    <li class="breadcrumb-item">Edit Manual Bill</li>
@endsection
        ...
        <div class="col-sm-12">
            <div class="card">
                <div class="card-header pb-0 d-flex justify-content-between align-items-center">
                    <h4 class="mb-0">Edit Manual Bill</h4>
                    <a href="{{ route('bill.index') }}" class="btn btn-dark btn-sm"><i class="fa fa-arrow-left"></i> Back to List</a>
                </div>
                
                <form action="{{ route('bill.update', $bill->id) }}" method="POST" id="edit_bill_form">
                    @csrf
                    <div class="card-body">
                        <div class="row mb-4">
                            <div class="col-md-3">
                            ...
                            <div class="col-md-2">
                                ...
                                <input type="date" name="bill_date" class="form-control" value="{{ date('Y-m-d', strtotime($bill->bill_date)) }}" required>
                            </div>
                            <div class="col-md-2">
                                @php 
                                    $days = 0;
                                    if($bill->due_date && $bill->bill_date) {
                                        $d1 = new DateTime($bill->bill_date);
                                        $d2 = new DateTime($bill->due_date);
                                        $days = $d2->diff($d1)->format("%a");
                                    }
                                @endphp
                                <label>Due (Days)</label>
                                <input type="number" name="due_days" class="form-control" value="{{ $days }}" placeholder="0">
                            </div>
                            <div class="col-md-3">
                            ...
                            <table class="table billing-table table-bordered">
                                ...
                                <tbody id="billing_tbody">
                                    ...
                                    <tr class="bill-row">
                                        ...
                                        <td><input type="text" name="items[{{ $index }}][description]" class="billing-input" value="{{ $item->description }}" placeholder="Description of Goods" required></td>
                                        <td><input type="number" step="0.001" name="items[{{ $index }}][qty]" class="billing-input amount-calc row-qty" value="{{ $item->qty }}" placeholder="0.000" required></td>
                                        <td>
                                            <select name="items[{{ $index }}][unit]" class="billing-input">
                                        ...
                                        <td><input type="number" step="0.01" name="items[{{ $index }}][rate]" class="billing-input amount-calc row-rate" value="{{ $item->rate }}" placeholder="0.00" required></td>
                                        <td><input type="number" step="0.01" name="items[{{ $index }}][amount]" class="billing-input row-amount" value="{{ $item->amount }}" readonly placeholder="0.00"></td>
                                        <td>
                                            <select name="items[{{ $index }}][gst_percent]" class="billing-input amount-calc row-gst">
                                        ...
                                        <td><input type="number" step="0.01" name="items[{{ $index }}][total_amount]" class="billing-input fw-bold row-total" value="{{ $item->total_amount }}" readonly placeholder="0.00"></td>
                                    </tr>
                                    @endforeach
                                </tbody>
                                <tfoot>
                                    ...
                                    <tr>
                                        ...
                                        <th id="grand_total_display" class="bg-dark text-white f-20">₹ {{ number_format($bill->grand_total, 2) }}</th>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
                    ...
                    <div class="card-footer text-end">
                        <button type="submit" class="btn btn-warning" id="btn_save">Update Bill</button>
                    </div>
                </form>
            </div>
        </div>
