...
@section('title', 'Cylinder Agent Master')
...
@section('breadcrumb-items')
    <li class="breadcrumb-item">Cylinder Agent Master</li>
@endsection
        ...
        <div class="row">
            <div class="col-12">
                ...
                <div class="card" id="add_type">
                    <form action="{{route('cylinder_agent.store')}}" method="POST" id="" class="modal-content" enctype="multipart/form-data">
                        @csrf
                        <div class="card-body row">
                            <div class="col-md-9">
                                <input type="text" name="name" id="name"   placeholder="Agent Name" oninput="this.value = this.value.toUpperCase()" class="form-control form-control-sm" required>
                            </div>
...
@section('script')
    <script>
        ...
        function get_datatable(page){
            // Use specific ID selector which is faster
            ...
            if ($container.length) {
                ...
                $.ajax({
                    url: '{{ route("cylinder_agent.datatable") }}',
                    data: {
                        page: page,
                        value: value,
                        search: search,
                    ...
                    success: function(data) {
                        $container.html(data);
                        // Initialize DataTable with minimal config for speed
                        $('#basic-test').DataTable({ 
                            dom: '{{ auth()->user()->role_as == "Admin" ? "Brt" : "rt" }}', 
                            "pageLength": -1, 
                            responsive: true, 
                                                                                    ordering: false // Disable client-side sorting if server-side is enough, faster
                        });
                    }
                });
            }
        ...
        // Bind value change separately (no debounce needed usually, but good to separate)
            ...
            $.ajax({
                ...
                success: function(data){
                    if(data.result == 1){
                        ...
                        // Clear inputs efficiently
                        $('#name').val('');
                        
                        $submitBtn.html('Add +').removeClass('disabled');
                        get_datatable(page);
                        $('#edit_modal').modal('hide');
                        $("#name").focus(); // Refocus on name for next entry
                    }else{
        ...
        function delete_agent(id){
            ...
            .then((willDelete) => {
                if (willDelete) {
                    var url = "{{route('cylinder_agent.delete',":id")}}";
                    url = url.replace(':id',id);
        ...
        function change_status(id){
            var url = "{{route('cylinder_agent.change_status',":id")}}";
            url = url.replace(':id',id);
            $.get(url, function(data){
                if(data.result == 1){
