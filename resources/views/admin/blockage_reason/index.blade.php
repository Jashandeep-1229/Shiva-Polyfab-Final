...
@section('title')
    Blockage Reason Master - {{ucfirst(request()->type)}}
@endsection
...
@section('breadcrumb-items')
    <li class="breadcrumb-item">Blockage Reason Master - {{ucfirst(request()->type)}}</li>
@endsection
        ...
        <div class="row">
            <div class="col-12">
                ...
                <div class="card" id="add_type">
                    <form action="{{route('blockage_reason.store')}}" method="POST" id="" class="modal-content" enctype="multipart/form-data">
                        @csrf
                        <input type="hidden" name="type" value="{{request()->type ?? 'printing'}}">
                        <div class="card-body row">
                            <div class="col-md-9">
                                <input type="text" name="name" id="name"   placeholder="Enter Reason" oninput="this.value = this.value.toUpperCase()" class="form-control form-control-sm" required>
                            </div>
    ...
    <script>
        ...
        function get_datatable(page){
            ...
            if ($container.length) {
                ...
                var page = page ?? 1;
                $.ajax({
                    url: '{{ route("blockage_reason.datatable") }}',
        ...
        function edit_modal(id,key_value){
            var url = "{{route('blockage_reason.edit_modal',":id")}}";
        ...
        function delete_blockage_reason(id){
            swal({
                title: "Are you sure?",
            ...
            .then((willDelete) => {
                if (willDelete) {
                    var url = "{{route('blockage_reason.delete',":id")}}";
                    url = url.replace(':id',id);
        ...
        function change_status(id){
            var url = "{{route('blockage_reason.change_status',":id")}}";
            url = url.replace(':id',id);
            $.get(url, function(data){
                if(data.result == 1){
