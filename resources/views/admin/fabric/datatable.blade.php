<div class="dt-ext table-responsive">
    <table class="display table-striped table-hover" id="basic-test">
        ...
        <tbody>
            @foreach ($fabric as $key => $item)
            <tr>
                <td>{{ $fabric->firstItem() + $key }}</td>
                <td>{{ $item->name ?? 'N/A' }}</td>
                <td>
                ...
                <td>
                    <div class="d-flex gap-1">
                        ...
                        <button type="button" onclick="openAddVendorModal('Fabric', {{$item->id}})" class="btn btn-primary btn-sm pointer p-1 f-12" title="Add Vendor">
                            <i class="fa fa-plus-circle"></i>
                        </button>
                        <button type="button" onclick="openVendorListModal('Fabric', {{$item->id}})" class="btn btn-info btn-sm pointer p-1 f-12" title="Vendor List">
                            <i class="fa fa-users"></i>
                        </button>
                        @endif
                        @if (auth()->user()->role_as == 'Admin')
                            <a onclick="delete_fabric({{$item->id}})" class="btn btn-danger btn-sm pointer p-1 f-12" title="Delete">
