...
@section('breadcrumb-items')
    <li class="breadcrumb-item">Fabric Master</li>
@endsection
        ...
        <div class="row">
            <div class="col-12">
                ...
                <div class="card" id="add_type">
                    <form action="{{route('fabric.store')}}" method="POST" id="" class="modal-content" enctype="multipart/form-data">
                        @csrf
...
@section('script')
    <script>
        ...
        function delete_fabric(id){
            ...
            .then((willDelete) => {
                if (willDelete) {
                    var url = "{{route('fabric.delete',":id")}}";
                    url = url.replace(':id',id);
                    $.get(url, function(data){
                        if(data.result == 1){
                            var page = Number($(".pages").find('span[aria-current="page"] span').text());
        ...
        function change_status(id){
            var url = "{{route('fabric.change_status',":id")}}";
            url = url.replace(':id',id);
            $.get(url, function(data){
                if(data.result == 1){
