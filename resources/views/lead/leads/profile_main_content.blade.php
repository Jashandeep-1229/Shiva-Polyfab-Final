<div class="card mb-4 border-0 shadow-sm">
    <div class="card-header pb-0 border-bottom bg-white d-flex justify-content-between align-items-center">
        <h5 class="fw-bold mb-0">All Enquiries For This Phone</h5>
        <div>
            @php $totalEnquiries = isset($allLeads) ? $allLeads->count() : (isset($siblingLeads) ? $siblingLeads->count() + 1 : 1); @endphp
            <span class="badge badge-light-primary">{{ $totalEnquiries }} Total</span>
        </div>
    </div>
    <div class="card-body">
        <div class="row g-3">
            {{-- All Leads in Fixed Order --}}
            @php
                $displayLeads = isset($allLeads) ? $allLeads : (isset($siblingLeads) ? collect([$lead])->concat($siblingLeads)->sortByDesc('id') : collect([$lead]));
            @endphp
            @foreach($displayLeads as $item)
            <div class="col-md-6 col-lg-4">
                @if($item->id == $lead->id)
                    {{-- Highlighted Current Card in Its Original Place --}}
                    <div class="card shadow-none mb-0 p-3 h-100 position-relative {{ ($item->status && $item->status->slug == 'won') ? 'anim-won' : (($item->status && $item->status->slug == 'lost') ? 'anim-lost' : 'border-primary') }}" 
                        ...
                        <h6 class="mb-1 fw-bold text-dark" style="font-size: 17px;">{{ $item->regarding ?? 'New Requirement' }}</h6>
                ...
                @else
                    {{-- Switchable Card in Its Original Place --}}
                    <div class="card shadow-none border mb-0 h-100 enquiry-card-hover {{ ($item->status && $item->status->slug == 'won') ? 'anim-won' : (($item->status && $item->status->slug == 'lost') ? 'anim-lost' : '') }}" 
                        ...
                        <h6 class="mb-1 fw-bold text-secondary" style="font-size: 16px;">{{ $item->regarding ?? 'Requirement' }}</h6>
                        <p class="text-muted mb-2" style="font-size: 14px;"><i class="fa fa-calendar-o me-1"></i> {{ $item->created_at->format('d M Y') }}</p>
                        <div class="d-flex justify-content-between align-items-center mt-auto pt-2">
...
<div class="card border-0 shadow-sm">
    <div class="card-header pb-0 border-bottom bg-white d-flex justify-content-between align-items-center">
        <h5 class="fw-bold">Lead Activity Timeline <small class="text-muted fw-normal ms-2">({{ $lead->lead_no }})</small></h5>
        @php 
    ...
    <div class="card-body" id="timeline-container">
        @include('lead.leads.timeline', ['lead' => $lead])
    </div>
</div>
